...
# App.py's location relative to /models varies by machine/setup, so search a
# few plausible candidates instead of hardcoding one path.
CANDIDATE_MODEL_DIRS = [
    os.path.join(BASE_DIR, 'models'),                     # models/ next to App.py
    os.path.join(BASE_DIR, '..', 'models'),                # models/ one level up
    os.path.join(BASE_DIR, 'marketing_campaign_ml', 'models'),  # models/ inside a subfolder
]

def _find_model_dir():
    for candidate in CANDIDATE_MODEL_DIRS:
        candidate = os.path.normpath(candidate)
        if os.path.isfile(os.path.join(candidate, 'regression_model.pkl')):
            return candidate
    tried = "\n".join(os.path.normpath(c) for c in CANDIDATE_MODEL_DIRS)
    raise FileNotFoundError(
        f"Could not find regression_model.pkl / classification_model.pkl. Tried:\n{tried}"
    )
...
with tab3:
    ...
    reg_df = pd.DataFrame({
        ...
        'R² Score' : ['77.11%','77.11%','77.11%','99.21%','99.20%'],
        'RMSE'     : ['₹2,22,882','₹2,22,882','₹2,22,882','₹41,483','₹41,744'],
        'Selected' : ['','','','','✅ Winner']
    })
